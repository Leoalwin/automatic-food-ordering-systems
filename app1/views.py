...
def customer_home(request):
    ...
    if request.method == "POST":
        ...
        if action == "Orderfood":
            ...
            last_order = (
                Order.objects.filter(customer=customer).exclude(status__exact="pending").
                order_by("-assigned_time", "-date").first()  )
            current_orders = [last_order] #if last_order else []
    ...
    context = {
        "customer": customer,"restaurants": restaurants,"show": show,"current_orders": current_orders,"order_history": order_history,
        "order": order,}
...
def restaurant_home(request):
    ...
    if request.method == "POST":
        ...
        if action == "Menu Details" :
            ...
            order = Order.objects.filter(restaurant=restaurants).select_related('customer')
        elif action == "select":
            order_id = request.POST.get("order_id")
            if order_id:
                selected_order = get_object_or_404(Order, pk=order_id)
        elif action == "Accept Order":
            accept_order = True
            orders = Order.objects.filter(restaurant=restaurants, status="Accept")

        elif action == "pick":
            order_id = request.POST.get("order_id")
            if order_id:
                order = Order.objects.filter(order_id=order_id, restaurant=restaurants, status="Accept").first()
                if order:
                    order.status = "Pickup" 
                    order.pickup_time = timezone.now()
                    order.save()
            orders = Order.objects.filter(restaurant=restaurants).exclude(status="Delivered")
        elif action == "Logout":
            logout(request)
            return redirect("login")
    return render(request, "restauranthome.html", {"menu": menu,"restaurant": restaurants,"show":show,"order": order, "drivers": drivers,
        "selected_order": selected_order,"current_datetime": current_datetime,"accept_order":accept_order,"orders":orders})
@login_required
     ...
     if request.method == "POST":
         ...
         if action == "Your Profile":
            show = True
             ...
             orders = Order.objects.filter(status="assigned")|Order.objects.filter(driver=delivery, status="Accept")
         elif action == "accept":
             order_id = request.POST.get("order_id")
             order = Order.objects.filter(order_id=order_id, status="assigned").first()
             if order:
                order.status = "Accept"
            ...
            current_order = True
            order = Order.objects.filter(driver=delivery, status="Pickup")

         elif action == "delivered":
            order_id = request.POST.get("order_id")
            order = Order.objects.filter(order_id=order_id, driver=delivery, status="Pickup").first()
            if order:
            ...
            past_orders = Order.objects.filter(driver=delivery)
         elif action == "Logout":
            logout(request)
            return redirect("login") 
     context = { "show": show ,"delivery":delivery,"orders":orders,"past_orders":past_orders,"assigned_orders":assigned_orders,"current_order":current_order,"order":order} 
